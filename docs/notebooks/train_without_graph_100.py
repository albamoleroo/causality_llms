...
os.makedirs('saved_models_no_graph_gt_order', exist_ok=True)
...
def train_and_save_selected_graph(model_idx):
    ...
    # MAantaining gt_order
    #order= np.random.permutation(n_features).tolist()
    ...
    model_path = f'saved_models_no_graph_gt_order/flow_no_graph{model_idx}.pt'
    torch.save({
    ...
    print(f"Saved model {model_idx} without graph structure to {model_path}")
