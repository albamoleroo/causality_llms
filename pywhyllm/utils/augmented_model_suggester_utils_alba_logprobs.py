...
def query_llm(variable1, variable2, source_text=True, retriever=True, llm=None, 
              return_prompt=False, confidence_level=True, log_probs=True,
              openai_client=None, model_name="gpt-4o-mini", temperature=0.3):
    """
    Query the LLM to determine causal relationship between two variables.
    ...
    Args:
        ...
        llm: Language model instance (if None, creates default OpenAI())
        return_prompt: If True, returns (response, system_prompt) tuple. Default: False
        confidence_level: If True, instructs LLM to provide confidence and strength scores. Default: False
        log_probs: If True, extracts logprobs from OpenAI API (requires openai_client). Default: False
        openai_client: OpenAI client for direct API access (required when log_probs=True)
        model_name: Model name for OpenAI client (used when log_probs=True)
        temperature: Temperature for generation (used when log_probs=True)
    
    Returns:
        If log_probs=False and return_prompt=False: String response from LLM
        If log_probs=False and return_prompt=True: Tuple (response, system_prompt)
        If log_probs=True: Dict with 'response', 'answer', 'confidence_score', 'strength_score',
                           'logprobs', 'answer_token_logprob', 'answer_choice_logprobs', etc.
    """
    
    # Build confidence instruction (used in both paths)
    confidence_instruction = ""
    if confidence_level:
        confidence_instruction = """ 
Additionally, provide TWO scores within tags:
...
"""
    ...
    # Build user query (used in both paths)
    query = f"""Determine the most likely cause-and-effect relationship between "{variable1}" and "{variable2}".
...
Provide your reasoning first, then give your final answer as ONLY the letter (A, B, or C) within <answer></answer> tags.{confidence_instruction}"""
    ...
    # ========== PATH 1: Direct OpenAI API with logprobs ==========
    if log_probs:
        ...
        # Build system prompt
        context_text = _get_context()
        if context_text:
            system_prompt = f"""You are a helpful assistant for causal reasoning.
    ...
    """
        ...
        # Make direct API call with logprobs
            ...
            if match:
                ...
                for start_idx, end_idx, token_item in token_spans:
                    ...
                    if overlaps_answer:
                        ...
                        # Clean token properly to handle ">A", "<answer", etc.
        ...
        return_dict = {
            ...
            'answer': answer_str,
            'confidence_score': confidence_score if confidence_level else None,
            'strength_score': strength_score if confidence_level else None,
        ...
        if return_prompt:
            return_dict['system_prompt'] = system_prompt
        
        return return_dict

    # ========== PATH 2: LangChain (original logic, no logprobs) ==========
    else:
        if llm is None:
            llm = OpenAI()
        
        if source_text:
            system_prompt_template = """You are a helpful assistant for causal reasoning.
    ...
    """
            ...
            if return_prompt:
                return response, system_prompt_template
            else:
                return response
